...
// References block used in review modals: always rendered so reviewers can
// see the section exists, with file links, an optional GitHub link, and an
// honest empty state when nothing was attached.
export function ReviewReferences({ contributionId, githubUrl }: { contributionId: string; githubUrl?: string | null }): JSX.Element { const [files, setFiles] = useState<ContributionFile[] | null>(null); useEffect(() => { (async () => { const f = await supabase.from('contribution_files').select('*').eq('contribution_id', contributionId); setFiles((f.data || []) as ContributionFile[]); })(); }, [contributionId]); return <div className="mt-4 border-t border-[#e5efec] pt-4"><p className="text-sm font-bold">References</p>{githubUrl && <a href={githubUrl} target="_blank" rel="noreferrer" className="mt-3 flex items-center gap-2 rounded-lg bg-white p-2.5 text-sm font-semibold text-[#087f78] hover:bg-[#e7f7f1]"><Github className="h-4 w-4" />Open GitHub repository</a>}{files === null && <p className="mt-3 text-sm text-[#8ca1a3]">Loading files…</p>}{files !== null && files.length === 0 && !githubUrl && <p className="mt-3 text-sm text-[#8ca1a3]">No files attached to this contribution.</p>}{files !== null && files.length > 0 && <div className="mt-2 space-y-2">{files.map((file) => <FileLink key={file.id} file={file} />)}</div>}</div>; }
