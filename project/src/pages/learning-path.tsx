...
import { Activity as ActivityIcon, Cloud, Code2, Eye, FileText, Pencil, Trash2, Upload, X } from 'lucide-react';
...
export function SyllabusSection({ profile }: { profile: Profile }): JSX.Element {
  ...
  const upload = async (): Promise<void> => {
    ...
    if (!isUploadAllowed(file)) { setMessage('That file type is not supported. PDF, Office documents, images, video, audio, ZIP archives, code files, and Google Docs exports are accepted.'); return; }
    if (file.size > 25 * 1024 * 1024) { setMessage('Keep syllabus files under 25 MB.'); return; }
...
export function NotesSection({ profile }: { profile: Profile }): JSX.Element {
  ...
  const submit = async (event: FormEvent): Promise<void> => {
    ...
    if (!title.trim() || !file) { setMessage('Add a note name and choose a file.'); return; }
    if (!isUploadAllowed(file)) { setMessage('That file type is not supported. PDF, Office documents, images, video, audio, ZIP archives, code files, and Google Docs exports are accepted.'); return; }
    if (file.size > 25 * 1024 * 1024) { setMessage('Keep note files under 25 MB.'); return; }
    setBusy(true);
    const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, '-');
...
export function NoteCard({ note, profile, onChanged }: { note: StudyNote; profile: Profile; onChanged: () => Promise<void> | void }): JSX.Element {
  ...
  async function quickDelete(): Promise<void> {
    ...
    if (error) { window.alert('We could not delete that note. Please try again.'); return; }
...
export function NoteManageModal({ note, onClose, onChanged }: { note: StudyNote; onClose: () => void; onChanged: () => Promise<void> | void }): JSX.Element {
  ...
  const replaceFile = async (): Promise<void> => {
    ...
    if (!isUploadAllowed(newFile)) { setMessage('That file type is not supported. PDF, Office documents, images, video, audio, ZIP archives, code files, and Google Docs exports are accepted.'); return; }
    if (newFile.size > 25 * 1024 * 1024) { setMessage('Keep note files under 25 MB.'); return; }
    setBusy(true); setMessage('');
