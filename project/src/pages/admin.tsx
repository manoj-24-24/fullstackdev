...
import { formatDate, friendlyError, isUploadAllowed, uploadFailedMessage, useLiveRefresh, UPLOAD_ACCEPT, uploadTypeHint } from '@/lib/ui';
...
import { ArrowLeft, BookOpen, Check, Eye, Github, GraduationCap, ListChecks, Mail, Megaphone, PencilLine, Plus, Send, ShieldCheck, Trash2, Upload, Users, X } from 'lucide-react';
import type { Role, Profile, Subject, Contribution, Feedback, AdminNote, NoteFeedbackItem, ContributionFile } from '@/types';
...
export function AdminDashboard({ profile, navigate }: { profile: Profile; navigate: (path: string) => void }): JSX.Element {
  ...
  const [editValues, setEditValues] = useState({ title: '', summary: '', description: '', code: '' });
  ...
  const openEdit = (item: Contribution): void => {
    setEditValues({ title: item.title, summary: item.summary || '', description: item.description || '', code: item.code || '' });
    setEditing(item);
  };
  const saveEdit = async (event: FormEvent): Promise<void> => {
    ...
    if (!editing) return;
    setBusy(true);
    const { error } = await supabase.from('contributions').update({ title: editValues.title, summary: editValues.summary, description: editValues.description || null, code: editValues.code || null }).eq('id', editing.id);
  ...
  const addSubject = async (event: FormEvent): Promise<void> => {
    ...
    setMessage(error ? friendlyError() : 'Subject added.');
    setNewSubject('');
    if (!error) await load();
  };
  const removeSubject = async (subject: Subject): Promise<void> => {
    if (!window.confirm(`Delete the subject “${subject.name}”? Contributions in it are kept but become uncategorized.`)) return;
    setSubjectBusy(true);
    const res = await supabase.rpc('delete_subject', { p_subject_id: subject.id });
    const failed = ((res.data as unknown as { error?: { message?: string } } | null)?.error?.message) || res.error?.message;
    setSubjectBusy(false);
    setMessage(failed ? failed : `Subject “${subject.name}” deleted.`);
    if (!failed) await load();
  };
...
// Attached files inside an admin modal (edit / review contribution): fetched
// fresh so admins always see what the student uploaded, plus an honest empty
// state when there are none.
export function ModalReferences({ contributionId }: { contributionId: string }): JSX.Element {
  const [files, setFiles] = useState<ContributionFile[] | null>(null);
  useEffect(() => {
    let alive = true;
    (async () => {
      const f = await supabase.from('contribution_files').select('*').eq('contribution_id', contributionId);
      if (alive) setFiles((f.data || []) as ContributionFile[]);
    })();
    return () => { alive = false; };
  }, [contributionId]);
  if (files === null) return <p className="mt-3 text-xs text-[#8ca1a3]">Loading files…</p>;
  if (!files.length) return <p className="mt-3 text-xs text-[#8ca1a3]">No files attached to this contribution.</p>;
  return <div className="mt-3 space-y-2">{files.map((file) => <FileLink key={file.id} file={file} />)}</div>;
}
